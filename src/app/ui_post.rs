                    ...
                    for (idx, item) in posts.iter_mut().enumerate() {
                        match item {
                            ...
                            FeedItem::Dehydrated { uri: _, height } => {
                                ...
                                // ⚡ Bolt: Only render spinners for visible dehydrated items to prevent
                                // constant 60 FPS repaints from off-screen animations, significantly reducing idle CPU usage.
